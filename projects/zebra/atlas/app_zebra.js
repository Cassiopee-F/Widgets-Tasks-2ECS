...
// OSM PRESETS
// ============================================================
// ── ZEBRA OSM PRESETS — équipements de sécurité piétons ──────────────────────
// Remplace les presets OSM génériques. Requêtes confirmées sur Marseille 2026-05-29.
const OSM_PRESETS = {
    // Critère 4 — cl_feu (628 nodes/6km² Marseille)
    traffic_signals: { name: 'Feux piétons',    icon: '🚦', category: 'signalization', model: 'traffic_light',
        query: '(node["highway"="traffic_signals"]; node["crossing"="traffic_signals"];)' },
    // Critère 12 — cl_arret_de_bus (94 nodes/6km²)
    bus_stops:       { name: 'Arrêts TC',        icon: '🚏', category: 'furniture',    model: 'bus_shelter',
        query: '(node["highway"="bus_stop"]; node["public_transport"="stop_position"]; node["railway"="tram_stop"];)' },
    // Critère 13 — cl_bande_d_eveil_de_vigilance (2452 nodes/6km²)
    tactile_paving:  { name: 'BEV podotactile',  icon: '⬜', category: 'accessibility', model: 'bollard',
        query: '(node["tactile_paving"~"yes|no|incorrect"]; node["kerb"];)' },
    // Critère 6 — cl_eclairage_public_moins_20m (223 nodes/6km²)
    street_lamps:    { name: 'Éclairage public', icon: '💡', category: 'lighting',     model: 'streetlamp',
        query: 'node["highway"="street_lamp"]' },
    // Critère 2 — cl_ralentisseurs (9 tables + 115 bumps/6km²)
    traffic_calming: { name: 'Ralentisseurs',    icon: '🔶', category: 'infrastructure', model: 'bollard',
        query: '(node["traffic_calming"~"table|bump|hump"]; way["traffic_calming"~"table|bump|hump"];)' },
    // Critère 9/10/11 — cl_presence_ilot (64 nodes/6km²)
    islands:         { name: 'Îlots refuge',     icon: '🏝️', category: 'infrastructure', model: 'bollard',
        query: 'node["crossing:island"="yes"]' },
    // Critère 3 — cl_manque_visibilite (8777 arbres/6km²)
    trees:           { name: 'Arbres masquants', icon: '🌳', category: 'vegetation',   model: 'tree_deciduous',
        query: '(node["natural"="tree"]; way["barrier"="hedge"];)' },
    // Critère 8 — cl_places_stationnement
    parking:         { name: 'Stationnement',    icon: '🅿️', category: 'infrastructure', model: 'car',
        query: '(node["amenity"="parking"]; way["amenity"="parking"];)' },
    // Voirie contextuelle (toujours utile)
    roads:           { name: 'Voirie',            icon: '🛤️', geomType: 'LineString',
        query: 'way["highway"~"primary|secondary|tertiary|residential|unclassified"]' },
    // Passages piétons OSM (référence)
    crossings:       { name: 'Passages piétons', icon: '🦓', category: 'signalization', model: 'bollard',
        query: '(node["highway"="crossing"]; way["highway"="crossing"];)' },
};

// ── Config service ZEBRA ──────────────────────────────────────────────────────
const ZEBRA_API = window.location.origin; // même origine que le widget

// Styles ZEBRA (depuis zebra_styles.json)
let ZEBRA_STYLES = null;
async function loadZebraStyles() {
    try {
        const r = await fetch(`${ZEBRA_API}/api/styles`);
        ZEBRA_STYLES = await r.json();
    } catch (_) {}
}

// ── Chargement données Pp depuis Grist → couche atlas ────────────────────────
async function loadPpLayerFromGrist(fieldToStyle = 'dangerosite_code') {
    if (!CONFIG.grist.ready) { showToast('Connectez ce widget à la table Pp', 'error'); return; }
    try {
        const records = await grist.docApi.fetchTable('Pp');
        const ids = records.id || [];
        if (!ids.length) { showToast('Table Pp vide — lancez une inférence', 'info'); return; }

        const DANGER_COLORS = { '3': '#d32f2f', '2': '#f57c00', '1': '#fbc02d', '0': '#2a9d8f', '-1': '#9e9e9e' };
        const DEG = 0.00004;

        const features = ids.map((id, i) => {
            const geomRaw = records.geometry_json?.[i];
            let coords = null;
            try { coords = geomRaw ? JSON.parse(geomRaw) : null; } catch (_) {}
            const lat = +records.lat?.[i] || 0, lng = +records.lng?.[i] || 0;
            if (!coords && lat && lng) {
                coords = [[lng-DEG,lat-DEG],[lng+DEG,lat-DEG],[lng+DEG,lat+DEG],[lng-DEG,lat+DEG],[lng-DEG,lat-DEG]];
            }
            if (!coords) return null;
            const code = records.dangerosite_code?.[i] ?? -1;
            return {
                type: 'Feature',
                geometry: { type: 'Polygon', coordinates: [coords] },
                properties: {
                    pp_id: records.pp_id?.[i],
                    id_zebra: records.id_zebra?.[i],
                    dangerosite_code: code,
                    dangerosite: records.dangerosite?.[i] || 'Non_evaluable',
                    lom_infraction: records.lom_infraction?.[i] || 'Non_evaluable',
                    statut_terrain: records.statut_terrain?.[i] || 'A_voir',
                    heading_deg: records.heading_deg?.[i] || 0,
                    commune: records.commune?.[i],
                    _gristId: id,
                    _color: DANGER_COLORS[String(code)] || '#9e9e9e',
                },
            };
        }).filter(Boolean);
        ...
        const existing = STATE.layers.find(l => l.id === 'layer-zebra-pp');
        if (existing) {
            existing.geojson = geojson;
            if (map.getSource(existing.id)) {
                map.getSource(existing.id).setData(geojson);
            }
            showToast(`${n} PP chargés`, 'success');
            return;
        }
        const layer = {
            id: 'layer-zebra-pp',
            ...
            style: {
                mode: 'categorized',
                colorField: fieldToStyle,
                categories: Object.entries({
                    3: '#d32f2f', 2: '#f57c00', 1: '#fbc02d', 0: '#2a9d8f', '-1': '#9e9e9e',
                }).map(([value, color]) => ({ value: +value, color })),
            },
        };
        ...
        addLayerToMap(layer);
        updateRailBadge();
        showToast(`${n} PP chargés depuis Grist`, 'success');
        // Centrer sur les PP
        if (features.length && map) {
            const lats = features.map(f => f.properties._gristId ? f.geometry.coordinates[0][0][1] : 0);
            const lngs = features.map(f => f.properties._gristId ? f.geometry.coordinates[0][0][0] : 0);
            // flyTo centroid
        }
    } catch (e) { showToast('Erreur chargement Pp: ' + e.message, 'error'); }
}
...
async function initGristTables() {
    // 1. Table atlas native (Maquette_Layers) — fetchTable est supporté par le plugin API standard
    try { await grist.docApi.fetchTable('Maquette_Layers'); }
    catch (_) {
        await grist.docApi.applyUserActions([['AddTable', 'Maquette_Layers', TABLE_SCHEMAS.Maquette_Layers]]);
    }
    // 2. Tables ZEBRA partagées (Pp, Etudes, Campagnes, Corrections)
    try {
    ...
    } catch (e) { console.warn('[ZEBRA atlas] schema init:', e.message); }
}
